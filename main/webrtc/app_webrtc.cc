...
void AppWebrtc::StartConnectOffer(const char *mac) {
    ...
    ESP_LOGI(TAG, "StartConnectOffer %s", mac);
...
void AppWebrtc::StartConnectAnswer(const char *mac) {
    ...
    ESP_LOGI(TAG, "StartConnectAnswer %s", mac);
    ...
    service_config.server_id = "e4:b0:63:b9:f2:f8";
    peer_signaling_set_config(&service_config);
    peer_signaling_join_channel();
