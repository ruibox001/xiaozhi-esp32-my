...
#define WEBRTC_PROTOCOL_SERVER_HELLO_EVENT (1 << 0)
...
bool WebrtcProtocol::OpenAudioChannel() {
    ...
    EventBits_t bits = xEventGroupWaitBits(event_group_handle_, WEBRTC_PROTOCOL_SERVER_HELLO_EVENT, pdTRUE, pdFALSE, pdMS_TO_TICKS(10000));
    if (!(bits & WEBRTC_PROTOCOL_SERVER_HELLO_EVENT)) {
...
void WebrtcProtocol::ParseServerHello(const cJSON* root) {
    ...
    xEventGroupSetBits(event_group_handle_, WEBRTC_PROTOCOL_SERVER_HELLO_EVENT);
}
